...
For result files produced before dali/runners/run_synthetic.py started
embedding prompt_text per record. Reads the source data/benchmark/tier2/*.jsonl
...
Usage:
    python -m dali.runners.backfill_prompt_text data/results/v0.2/2026-05-26/

    # Or specific files:
    python -m dali.runners.backfill_prompt_text data/results/v0.2/2026-05-26/openai_fast.json
"""
...
def load_prompt_map() -> dict[str, str]:
    """Build {prompt_id: prompt_text} from every data/benchmark/tier2/*.jsonl file."""
    repo_root = pathlib.Path(__file__).resolve().parent.parent
    synthetic_dir = repo_root / "benchmarks" / "tier2"
    if not synthetic_dir.is_dir():
        raise SystemExit(f"data/benchmark/tier2/ not found at {synthetic_dir}")
...
def main(argv: list[str]) -> int:
    ...
    print(f"Loading prompts from data/benchmark/tier2/ ...")
