...
Usage:
    ...
    python -m dali.runners.run_synthetic --list-models
...
from dali.runners.models import (
    assert_provider_ready,
    call_model_result,
    classify_model_error,
)
from dali.runners.model_registry import resolve_model, list_models, DEFAULT_DEV_MODELS
from dali.scoring.support import MAX_TOKENS, get_scorer_model, score_support
from dali.scoring.verification import inspect_citation_extraction, verify_citations
    ...
    if parsed_candidates:
        ...
        for vc in verified:
            ...
            if snap and snap.exists_verified and snap.extracted_text:
                try:
                    from dali.scoring.support import extract_claim_for_citation
...
async def main() -> None:
    ...
    parser.add_argument(
        "--prompts",
        type=Path,
        default=Path("data/benchmark/tier2"),
        help="Path to synthetic prompt directory (default: data/benchmark/tier2/)",
    )
    parser.add_argument(
        "--output",
        type=Path,
        default=Path(f"data/results/v0.2/{datetime.now().strftime('%Y-%m-%d')}"),
