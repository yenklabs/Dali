...
def lint(record_json: str) -> str:
    ...
    data/benchmark/tier1/corpus/citation_failure_cases.json. Returns a validation
...
def score(record_json: str) -> str:
    ...
    The MCP equivalent of ``python -m dali.runners.run_integrity``. Returns the
...
def probe(prompt_json: str) -> str:
    ...
    Returns:
        JSON string with keys:
            ...
            issues (list[str])
            summary (str)
            destination_file (str) - which data/benchmark/tier2/ file to add this to
    """
