...
`score` is the one that matters most — it runs the same code path as `python -m dali.runners.run_integrity` and surfaces the cryptographic lineage that anchors Dali's reproducibility claim. No terminal required.
...
Pick the one you use. Each config below uses `python -m tools.mcp` — the server itself runs via the `tools.mcp` Python package that you just installed.
...
> I edited the Park v. Kim record in `data/benchmark/tier1/corpus/citation_failure_cases.json`. `replay` it and confirm the replay_hash still matches its prior value. If it doesn't, that means my edit changed the verdict — show me what's different.
...
Each MCP tool has a CLI counterpart with the **same name** under `python -m tools.cli`. Same code path, same cryptographic hashes.

| MCP tool | Terminal equivalent |
|---|---|
| `lint` | `python -m tools.cli lint [corpus.json]` |
| `score` | `python -m tools.cli score [corpus.json] [--output …]` |
| `replay` | `python -m tools.cli replay [corpus.json] [--output …]` |
| `probe` | `python -m tools.cli probe <prompt.json or .jsonl>` |
| `draft` | `python -m tools.cli draft --category … --subcategory … --difficulty …` |
| `pack` | `python -m tools.cli pack <prompt.jsonl> [more.jsonl …]` |

`python -m dali.runners.run_integrity` remains the canonical underlying entry point and continues to work; the CLI is a thin wrapper.
...
- **Methodology** — the scoring rubric and policy versioning: [METHODOLOGY.md](../../docs/METHODOLOGY.md)
