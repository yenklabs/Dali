...
12-class closed vocabulary from `dali/corpus/taxonomy.py`:
...
Records must carry a verifiable `source_url` and seven other required fields to be scoring-eligible. `annotation_confidence` does not substitute for a verifiable source. See `dali/corpus/validator.py` for the complete gate.
...
Tier 1 records are canonical cases, such as Mata v. Avianca, stored as structured corpus records. Tier 2 records are prompt probes stored under `data/benchmark/tier2/` and used to test model or retrieval behavior against controlled citation tasks.
...
Each result file (`data/results/v0.2/{date}/{model_id}.json`) contains one JSON object per prompt:
