...
- writes a reproducible JSON artifact under `data/results/demo/`
...
Tier 2 uses the built-in 150-prompt synthetic corpus under `data/benchmark/tier2/`.
...
What this does:
- runs every prompt under `data/benchmark/tier2/` against the selected models
...
[06/25] adversarial_006 ... 3 citations extracted (citations_found)  exist=33%  support=0.00
   ...
   │      └─ prompt ID from data/benchmark/tier2/ corpus
   └─ progress counter
```
...
python -m dali.runners.run_synthetic \
  ...
  --output data/results/v0.2/$(date +%Y-%m-%d)/custom-synthetic.json
```
...
Create a JSONL entry under `data/benchmark/tier2/` that matches the repo schema:
...
```bash
jq '.[] | {prompt_id, model_id, citation_count, existence_rate, mean_support_score}' data/results/v0.2/$(date +%Y-%m-%d)/*.json
```
