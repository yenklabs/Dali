...
Usage:
    python -m dali_cli <verb> [args]

The CLI is a thin convenience wrapper. ``runners/run_integrity.py`` remains
...
# Ensure the repo root is importable when invoked as ``python -m dali_cli``
sys.path.insert(0, str(Path(__file__).resolve().parent.parent))

DEFAULT_CORPUS = "benchmarks/tier1/corpus/citation_failure_cases.json"
DEFAULT_OUTPUT = "results/demo/integrity.json"
...
def _cmd_score(args: argparse.Namespace) -> int:
    """Run the Tier 1 evaluator. Delegates to runners/run_integrity.py."""
    from runners.run_integrity import main as run_integrity_main
...
def _cmd_probe(args: argparse.Namespace) -> int:
    ...
    from dali_mcp.tools.prompt_tools import _check_prompt_impl
...
def _cmd_draft(args: argparse.Namespace) -> int:
    ...
    from dali_mcp.tools.prompt_tools import _new_prompt_impl
...
def _cmd_pack(args: argparse.Namespace) -> int:
    ...
    from dali_mcp.tools.prompt_tools import _bundle_prompts_impl
